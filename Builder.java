...
public class Builder {
	...
	Button compile;
	Button save;
	Image buildTut = new Image(getClass().getResourceAsStream("buildTut.png"));
	...
	public VBox makeBuilder(){
		...
		vBoxBuild.getChildren().addAll(new ImageView(buildTut),label, list, vBoxWipeable);
		return vBoxBuild;
	}//makeBuilder
	...
	public void fillOptions(String choice){
		if(choice == "tophat"){
			optionArray = FXCollections.observableArrayList(
					...
					"-m", "-i", "--segment-mismatches", "--min-segment-intron", "--b2-sensitive", "--b2-very-sensitive",
					"reference", "read", "--segment-length"
					);
		}else if(choice == "gatk"){
